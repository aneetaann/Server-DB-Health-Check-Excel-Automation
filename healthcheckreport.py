...
try:
    ...
    try:
        ...
        query = """
        select count(1) from {tablename} where DOCTIMESTAMP > '{YEST_DATE} 12:00:00' and doctimestamp <='{CURR_DATE} 12:00:00'
        UNION ALL
        select count(1) from {tablename} where UserStatus LIKE 'ERR%' and doctimestamp > '{YEST_DATE} 12:00:00' and doctimestamp <='{CURR_DATE} 12:00:00'
        UNION ALL
        select count(1) from {tablename} where UserStatus LIKE 'COMPLETE%' and RoutingStatus LIKE 'REPROCESSED%' and doctimestamp >'{YEST_DATE} 12:00:00' and doctimestamp <='{CURR_DATE} 12:00:00'
        UNION ALL
        select count(1) from {tablename} where UserStatus LIKE 'QUEUED%' or UserStatus like 'SUBMITTED%' and doctimestamp > '{YEST_DATE} 12:00:00' and doctimestamp <='{CURR_DATE} 12:00:00'
        """.format(tablename='xyz.bisdoc', CURR_DATE=curr_date, YEST_DATE=yest_date)
        #execute query
...
if yest_weekday == 'Sunday':
    sheet['B4'] = "Report Date : " + str(curr_weekday) + " - " + str(report_date) + " 09:00 AM EST                  Duration - From " + str(fri_weekday) + " - " + str(fri_date) + " 08:00 AM EST to " + str(curr_weekday) + " - " + str(report_date) + " 08:00 AM EST."     
    b = sheet['B4']
    b.font = Font(name='Calibri', size=10, bold=True, color="B30000")
else:
    sheet['B4'] = "Report Date : " + str(curr_weekday) + " - " + str(report_date) + " 09:00 AM EST                  Duration - From " + str(yest_weekday) + " - " + str(yester_date) + " 08:00 AM EST to " + str(curr_weekday) + " - " + str(report_date) + " 08:00 AM EST."     
    b = sheet['B4']
    b.font = Font(name='Calibri', size=10, bold=True, color="B30000")

#cell D16
sheet['D16'] = "Transaction Volume and status in TN (" + str(yes_date) + " 08:00 AM - " + str(today_date) + " 08:00 AM)"
d = sheet['D16']
